  ...
  return (
    <TouchOptimized
      ...
      className={`group relative bg-white rounded-2xl border border-slate-200 
        ...
        ${!isTouch ? 'hover:scale-[1.04] hover:-translate-y-1' : 'active:scale-[0.98]'}
        cursor-pointer
    ...
    >
      <div>
        ...
        <div className={`p-6 pt-7 flex flex-col h-full ${isMobile ? 'p-4 pt-5' : ''}`}>
          <div className="flex items-center mb-3">
            <div className={`${isMobile ? 'w-12 h-12' : 'w-14 h-14'} rounded-xl bg-gradient-to-br ${categoryColors[tool.category]} flex items-center justify-center shadow-md`}>
              <Icon className={`${isMobile ? 'w-6 h-6' : 'w-8 h-8'} text-white`} />
            </div>
            <div className="flex-1 ml-3">
              <h3 className={`font-semibold ${isMobile ? 'text-base' : 'text-lg'} text-slate-800 group-hover:text-blue-700 transition-colors`}>
                {tool.name}
              </h3>
          ...
          <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-slate-600 leading-relaxed mb-4 flex-1 ${isMobile ? 'line-clamp-2' : ''}`}>
            {tool.description}
          </p>
